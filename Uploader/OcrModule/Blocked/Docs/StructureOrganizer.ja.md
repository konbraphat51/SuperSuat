...
sequenceDiagram
    ...
    alt 見出しが1つも無い
        ...
        Leveler->>Leveler: 見出しをページごとにまとめ、MAX_PAGES_PER_REQUEST ページずつの部に分割
        loop 各部を文書順に
            Leveler->>Leveler: 確定済みの各レベルにつき1ページを例として選ぶ
            Leveler->>Leveler: _build_messages（プロンプト + 例のページ + 確定済みレベル + この部のページ + その見出しのJSON）
            loop 部内の全見出しにレベルが付くまで、最大 MAX_ATTEMPT_COUNT 回
...
見出しのあるページはいずれも画像であるため、見出しが100ページに散る文書を1リクエストに
収めることはできない。そこで見出しページを文書順に `MAX_PAGES_PER_REQUEST` ページずつの
部に分けて処理する。

ただ分割しただけでは、各部が自分の見出しだけを見てゼロから順位付けするため、部と部で
基準が揃わない。同じ大きさの見出しが、ある部では2、次の部では3になってしまう。そのため
第2部以降には**確定済みのレベルごとに1ページ**を、どのブロックがそのレベルの例なのかを
明示した上で添え、併せて確定済みレベルのJSONも渡す。「レベル2がどう組まれていたか」を
モデルに記憶させるのではなく、その現物を見せる。これらのページは判断基準であって回答
対象ではないことも明示する。1ページが2つのレベルの例を兼ねることも多く、その場合は
1回だけ送る。

回答はorderではなく「見出しごとのレベル」の形で受け取る。この段階が変更するのは1つの
フィールドだけであり、1つの部をまとめて答えさせることが部内の一貫性を生むため。1未満の
