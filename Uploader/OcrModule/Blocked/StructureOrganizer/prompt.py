...
3. Every figure on the page has had its caption settled: tied to the text block holding its caption, or explicitly recorded as having none. A figure you say nothing about is not finished, since "no caption" and "not looked at yet" have to be told apart.
4. A paragraph that the page boundary cut in two is one block again. The block detector works page by page, so a paragraph running from the previous page onto this one arrives as two blocks; the printed line break between them is only where the page ended, not something the author wrote.
5. The blocks are in reading order: the order a person reads them in, not the order the block detector happened to find them in. This matters most on multi-column pages, and where a figure sits among the text.
...
- merge_blocks(former_block_id, latter_block_id, join_with_space): join two text blocks that are really one, keeping the text on the former block and removing the latter. This is what the first paragraph of page {page_number} needs when it is the rest of a paragraph the previous page broke off in the middle - give the previous page's block as former_block_id and this page's as latter_block_id. Set join_with_space true where the language separates words with spaces (English etc.) and false where it does not (Japanese, Chinese); if the break falls inside a hyphenated word split across the pages, merge without a space and drop the hyphen with a following edit_block.
...
Set is_last_batch to true when page {page_number} is completely done: every text block on it labeled, every heading on it leveled, every figure on it given a set_caption - whether to a caption block or to null - and the reading order right.
