...
4. A block that the page boundary cut in two is marked as continuing from the previous page. The block detector works page by page, so a paragraph running from the previous page onto this one arrives as two blocks; the printed line break between them is only where the page ended, not something the author wrote.
